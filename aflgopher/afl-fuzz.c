...
/* edited */
static int target_visited = 0;      /* number of target reached         */
static int target_total  = 0;      /* number of target in total        */
static int *target_heap_ptr;
...
void read_target_total() {
  ...
  strcat(target_dir,"/Itargets.txt");
  fp=fopen(target_dir,"r");
  if(fp==NULL) {
  	PFATAL("Unable to open 'Itargets'");
  }
  ...
  target_total=linesCount;
  int target_heap[target_total];
  target_heap_ptr=target_heap;
...
static inline u8 has_new_bits(u8* virgin_map) {
  ...
  u64* target_reached = (u64*) (trace_bits + MAP_SIZE + 16);
  ...
  u32* target_reached = (u32*) (trace_bits + MAP_SIZE + 8);
  int target_num=(int) (*target_reached);
  ...
  FILE *fp;
  fp=fopen(profiler_path, "r");
  char s[100];
  char c[100];
  fscanf(fp, "%s", s);
  while (fscanf(fp, "%s %d", c, &cur_index)!=EOF){
  	if (strcmp(c, "BB")==0){
  		total_count++;
	  	if (cur_index-1> table_size)
	  		FATAL("distance table overflow: %d  %d",total_count, cur_index);
	  	total_distance+= distance_table[cur_index -1];
  	}
  }
  
  fclose(fp);
  //clear content
  fclose(fopen( profiler_path, "w"));

  if (total_count > 0) 
    cur_distance = (float) (total_distance) / (float) (total_count);
  else
    cur_distance = -1.0;
...
static u8* describe_op(u8 hnb) {
  ...
  if (syncing_party) {
    ...
    sprintf(ret, "time:%llu,src:%06u", get_cur_time() - start_time, current_entry);
...
#define SP20    SP10 SP10
  ...
  */
  ...
  sprintf(tmp, "%0.02f", min_distance);



  SAYF(bV bSTOP " last uniq crash : " cRST "%-34s " bSTG bV bSTOP
       " min distance : %s%-6s " bSTG bV "\n",
       DTD(cur_ms, last_crash_time), unique_crashes ? cLRD : cRST,
       tmp);
  ...
  */
  ...
  sprintf(tmp, "%d ", target_visited);
         
  SAYF(bV bSTOP "  last uniq hang : " cRST "%-34s " bSTG bV bSTOP 
       "target visited: " cRST "%-6s " bSTG bV "\n",
       DTD(cur_ms, last_hang_time), tmp);
  ...
  /* Edited: stop when find a unique crash */
  
  //if (unique_crashes >= 1) FATAL("unque crash found");
...
int main(int argc, char** argv) {
  ...
  fclose(fp);
  //test map
  OKF("the value of 196  %f \n", distance_table[196]);
    
  // set trace file
  /*
  strcpy(profiler_path,"AFLGO_PROFILER_FILE=");
  strcat(profiler_path, getenv("PWD"));
  strcat(profiler_path, "/");
  strcat(profiler_path, out_dir);
  strcat(profiler_path,"/trace");
  putenv(profiler_path);
  SAYF("%s \n", getenv("AFLGO_PROFILER_FILE"));
  */
  
  strcpy(profiler_path, getenv("AFLGO_PROFILER_FILE"));
  fclose(fopen( profiler_path, "w"));
